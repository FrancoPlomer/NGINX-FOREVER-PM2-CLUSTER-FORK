...
if(puerto._.includes('CLUSTER')) {
    exec('pm2 start server.js --name="Server1" --watch -i max -- 8081', async (error, stdout, stderr) => {
        if (error) {
    ...
    exec('forever list', async (error, stdout, stderr) => {
        if (error) {
            console.log(`error: ${error.message}`);
            return;
        }
        ...
        console.log(`listando procesos por forever: ${stdout}`);
    });
    exec('tasklist', async (error, stdout, stderr) => {
        ...
        console.log(`listando procesos por sistema operativo: ${stdout}`);
    });
}
else{
    exec('pm2 start server.js --name="Server1" --watch -- 8081', async (error, stdout, stderr) => {
        if (error) {
            console.log(`error: ${error.message}`);
            return;
        }
...
function validatePort (PORT) {
    return typeof(PORT) === 'number' ? PORT : config.PORT;
}
